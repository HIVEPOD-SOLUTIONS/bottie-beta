        ...
        try {
          ...
          return {
            ...
            tip: isAddressBased
              ...
              : isTopup
                ? `A payment confirmation card is shown to the user. Do NOT say payment was sent — wait. When you receive {paid:true} from this tool, IMMEDIATELY call poll_bitrefill_order(invoiceId="${invoice.invoice_id}", isTopup=true). Airtime credited directly to the phone; receipt only to ${recipientEmail}.`
                : `A payment confirmation card is shown to the user. Do NOT say payment was sent — wait. When you receive {paid:true} from this tool, IMMEDIATELY call poll_bitrefill_order(invoiceId="${invoice.invoice_id}"). Code will be emailed to ${recipientEmail} after Bitrefill confirms.`,
          };
        } catch (err: unknown) {
