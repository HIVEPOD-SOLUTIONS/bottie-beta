...
function usdToAtomicUsdc(usd: number, decimals: number): string {
  if (decimals < 2) {
    throw new Error(
      `usdToAtomicUsdc: unexpected decimals ${decimals} (expected USDC-like, >= 2)`,
    )
  }
...
async function attemptChain(
  ...
  const priceUsd =
    invoice.payment_info?.altcoinPrice ?? invoice.payment_info?.amount
  ...
  const atomicTarget = usdToAtomicUsdc(
    priceUsdNum,
    USDC_DECIMALS[underlyingMethod],
  )
  ...
  try {
    ...
    if (!swapAmountXrp) {
      ...
      console.error(
        `[xrp-purchase] ${underlyingMethod}: quote had no usable amountIn`,
        q,
      )
      return { ok: false, reason: 'chain_unavailable' }
    }
    ...
    if (isAmountTooLowError(err)) {
      console.error(
        `[xrp-purchase] ${underlyingMethod}: NEAR Intents rejected as too low to bridge ($${priceUsdNum.toFixed(2)})`,
      )
      return { ok: false, reason: 'amount_too_low', priceUsdNum }
    }
...
export async function initiateXrpPurchase(opts: {
  ...
  const candidates = opts.underlyingMethod
    ? [opts.underlyingMethod]
    : FALLBACK_PRIORITY
  ...
  if (underlyingMethod !== candidates[0]) {
    console.log(
      `[xrp-purchase] fell back to ${underlyingMethod} (${candidates[0]} unavailable)`,
    )
  }
 ...
 */
export async function getOrCreateSidebarWallet(
  userId: string,
  ...
  const [existing] = await db
    ...
    .limit(1)
  if (existing)
    return { id: existing.walletRequestId, address: existing.address }
 ...
 */
export async function walletRequestBelongsToUser(
  walletRequestId: string,
  userId: string,
): Promise<boolean> {
  const [sidebarMatch] = await db
    ...
    .where(
      and(
        eq(xrplSidebarWallets.walletRequestId, walletRequestId),
        eq(xrplSidebarWallets.userId, userId),
      ),
    )
    .limit(1)
  ...
  const [orderMatch] = await db
    ...
    .where(
      and(
        eq(bitrefillOrders.xrplWalletRequestId, walletRequestId),
 ...
 */
export async function listRecoverableXrpOrders(
  userId: string,
): Promise<RecoverableXrpOrder[]> {
    ...
    candidates.map(async (order): Promise<RecoverableXrpOrder | null> => {
      try {
        ...
        if (order.status === 'pending') {
          const failStatus =
            wallet.swapStatus === 'EXPIRED' ? 'expired' : 'failed'
          await markXrpPurchaseFailed(
            order.xrplWalletRequestId!,
            failStatus,
          ).catch(() => {})
        }

        const recoverableXrp = wallet.swapAmountDrops
          ? Number(wallet.swapAmountDrops) / 1_000_000
          : 0
...
export async function recoverXrpOrder(
  ...
  if (!order)
    throw new Error(
      "That payment wasn't found, or doesn't belong to this account.",
    )
  if (order.xrplRecoveredAt)
    throw new Error("This payment's funds were already recovered.")

  const wallet = await getWalletRequest(walletRequestId)
  if (!FAILURE_SWAP_STATUSES.has(wallet.swapStatus)) {
    throw new Error(
      "This payment hasn't failed (or already completed) — there's nothing to recover yet.",
    )
  }
  const recoverableXrp = wallet.swapAmountDrops
    ? Number(wallet.swapAmountDrops) / 1_000_000
